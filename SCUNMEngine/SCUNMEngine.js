...
module.exports = initEngine(); //build engine and return its ctor fnc.
...
function initEngine() {
	...
	function Engine(assets) {
		...
		assets.state = new Proxy(assets.state, getProxyHandler(this));//proxy game state to emit state change events
	...
	Engine.prototype.execCommand = function (verb, dObject, iObject) {
		...
		if (updatedState) this.emit(stateChanged, JSON.stringify(this.getState()));
		return outPut;
	};
...
function getProxyHandler(engine) {
	...
	return {
		set: function (target, key, value) {
			target[key] = value;
			updatedState = true; //deferred event emmiting, more changes could be comming
		},
